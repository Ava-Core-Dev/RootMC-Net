#!/usr/bin/env python3
"""Opt-in safe auto-push for the RootMC Net Vercel checkout."""
...
DENY = (".env", "credentials", ".runtime/", "node_modules/", ".next/", "dist/", "build/", "coverage/")
...
def allowed(path: str) -> bool:
    ...
    return not any(value == item or value.startswith(item) or item in value for item in DENY)
...
def main() -> int:
    ...
    try:
        ...
        denied = [name for name in names if not allowed(name)]
        if denied:
